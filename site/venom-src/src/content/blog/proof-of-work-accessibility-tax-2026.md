...
The same proof-of-work wall that costs about 2 seconds on a flagship laptop can cost up to 2 minutes on an old phone. At the same difficulty, on the same site, one visitor barely notices and another stares at a spinner long enough to give up. The AI scraper the wall was built to stop pays close to nothing.

Proof-of-work anti-scraping is sold as a fair trade: humans pass for free, bots pay a tax that adds up at scale. The arithmetic is more honest once you ask which humans, and which bots.

Anubis is the most widely deployed example. It asks the visitor's browser to find a hash with a required number of leading zero hex digits (nibbles) before serving the page. Mike Bommarito measured the default difficulty 4 at about 1.35 seconds on an Intel Core Ultra 7 165H running headless Chromium, at a hash rate near 87,600 hashes per second<sup><a href="#ref-1">1</a></sup>.

That is the fast end of the distribution. The slow end is where the argument lives. At difficulty 5, reported solve times spread across roughly two orders of magnitude: a modern MacBook around 2 seconds, an older notebook tens of seconds, and a mobile phone up to about 2 minutes<sup><a href="#ref-2">2</a></sup>. These are reported times consolidated from device benchmarks rather than one controlled table, so treat them as a range. The shape of the range is the point. The same wall, at the same setting, taxes one visitor 2 seconds and another 2 minutes.

That spread is not random. Slow devices belong to people on older phones, budget hardware, and metered connections, populations that correlate with lower income and with the Global South. The traffic the wall exists to stop does not. One consolidated analysis reports that the large majority of AI crawler traffic, on the order of 90 percent, originates from North American infrastructure<sup><a href="#ref-2">2</a></sup>. We have not traced that figure to its underlying measurement, so treat it as an estimate; the direction of the imbalance, not the exact percentage, carries the argument. The people most taxed are not the people the tax is aimed at.
...
The curve is steep, and theory and measurement diverge in a way worth keeping straight. In theory, each added difficulty level requires one more leading zero hex digit, which means searching a hash space 16 times larger (one hexadecimal nibble). Expected work per level is 16x, not the often-repeated 10x. Measured wall-clock times scale more slowly, because fixed browser and JavaScript overhead dominates at low difficulty. The Bommarito benchmark reports difficulty 1 under 100 milliseconds, difficulty 4 around 1.35 seconds, difficulty 8 around 11 seconds, and difficulty 10 around 114 seconds on capable hardware<sup><a href="#ref-1">1</a></sup>. That is roughly 84x from difficulty 4 to difficulty 10 across six levels, closer to an order of magnitude per level near the high end than a clean 16x. The practical lesson holds either way. A small difficulty bump, chosen by an operator to fend off a crawler surge, multiplies the wait for every visitor and multiplies it most for the visitor whose device is already slow.

A hardware twist bends the curve further against humans. Anubis uses plain SHA-256, which runs efficiently on GPUs. A scraper operator with GPUs gains a reported throughput advantage of 100x or more over a commodity browser hashing in JavaScript<sup><a href="#ref-2">2</a></sup>. We cite that as reported rather than independently benchmarked, but the direction is not in doubt given SHA-256's known GPU friendliness. The function meant to be expensive for the adversary is, by construction, cheaper for the adversary who can afford specialized hardware than for the human stuck in a browser tab.
...
The third exclusion is the sharpest, because it pits one defensive value against another. Anubis can require users to disable anti-fingerprinting extensions such as JShelter, and it can redirect after solving without letting the user re-enable that protection<sup><a href="#ref-2">2</a></sup>. A defense premised on JavaScript execution and on signals that resemble fingerprinting collides head-on with the privacy tooling that protects the cautious users least able to absorb risk. Add a battery and thermal cost: a phone grinding for two minutes at difficulty 5 drains power and heats up, a charge a desktop user never notices<sup><a href="#ref-2">2</a></sup>.

## The receipts

These are not hypothetical edge cases. They have names and dates.

On GNOME's GitLab, users reported Firefox stuck calculating a challenge for 30 minutes at difficulty 4, while Chrome cleared the same difficulty in seconds. Some users were blocked entirely<sup><a href="#ref-4">4</a></sup>. The magnitude gives it away as an outlier: 30 minutes is three orders of magnitude past the slow-end estimate for ordinary devices, so it reflects a Firefox calculation pathology, an engine-dependent hang, not a representative point on the cost curve. What it illustrates is brittleness. Identical difficulty, one engine clearing in seconds and another stalling for half an hour, a failure the visitor did not choose and cannot diagnose.
...
Memory-hard challenges narrow the hardware gap. ALTCHA uses Argon2 or Scrypt, which are memory-bound rather than compute-bound like plain SHA-256<sup><a href="#ref-7">7</a></sup>. Memory-bound functions resist the GPU and ASIC speedups that let a well-funded scraper outpace a browser by 100x, shrinking the gap between a commodity client and optimized attacker hardware. The trade-off: memory-hard work can be heavier on the constrained mobile devices the regressive curve already punishes, so it must be tuned, not assumed.
...
This is the evidence-based nuance to the claim, made in our [cost imposition versus value degradation analysis](/blog/cost-imposition-vs-value-degradation/), that proof-of-work friction is trivial for human users. It is trivial for some humans. For the user on an old phone, the screen-reader user, the curl script, and the privacy-extension user, it is not. (That companion post draws the same Bommarito benchmark. Measured from Anubis's lowest difficulty, the multiple runs into the hundreds; the roughly 84x cited here is measured from the difficulty-4 default to difficulty 10. Same data, different baseline. That is exactly why this post states the cost as a measured range tied to a named device and difficulty rather than one headline multiplier.) For the server-side picture of how operators fare a year into deployment, see [Anubis at one year](/blog/anubis-at-one-year/).

The open question is not whether operators should defend against industrial crawling. They clearly must. It is whether the next generation of challenges can be designed so the bill lands on the adversary it was written for, not on the visitor with the slowest device in the room. The benchmarks to answer that already exist. Measure the mechanism from both ends, report impact by device class and assistive-technology path instead of aggregate block rate alone, and you can tell a defense that earns its place from one that quietly shifts its bill onto the least-resourced visitor. Apply that test before deploying proof-of-work.
